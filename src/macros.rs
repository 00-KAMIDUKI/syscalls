...
macro_rules! define_syscalls {
    (
        ...
        #[inline]
        $pub unsafe fn $name($($iden: $type),*) -> Result<$ret, $crate::Errno> {
            $crate::syscall!($crate::Sysno::$name, $($iden),*).map(|x| x as $ret)
        }
    };
    (
        $pub: vis
        ...
        #[inline] $pub unsafe fn $name($($iden: $type),*) -> Result<(), $crate::Errno> { $crate::syscall!($crate::Sysno::$name, $($iden),*).map(|_| ())
        }
    };
    (
        $(
